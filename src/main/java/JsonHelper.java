...
public class JsonHelper {
    String status; //переменная для поля status в ответе
    List results; //переменная для поля results в ответе
    public static String getBaseUrl( String configPath ) {  //создаем первую часть ссылки с endpoint + key (переделать потом)
        String baseUrl;
        byte[] mapData;
    ...
    public static JsonHelper getResponseParam( String response ) {   // получаем из ответа значение двух полей results и status
        JsonHelper resp = new JsonHelper();
        byte[] mapData;
    ...
    public String createUrl( String location,  // создаем второую половину ссылки из параметров
