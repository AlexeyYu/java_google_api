...
@RunWith(DataProviderRunner.class) //указываем запускатель тестов
public class ParameterizedTest {


    @DataProvider //указываем перечень параметров
    ...
    @Test
    @UseDataProvider("dataForPositiveTest") //указываем тестовый метод и имя используемого дата провайдера
    public void positiveTest( String location, String radius, String keyword,
        ...
        assertEquals(200, response.getStatus()); //асертим статус код сервера
        assertEquals(statusMessage, JsonHelper.getResponseParam(response.getBody()).status); //асертим параметр status
        assertNotEquals(0, JsonHelper.getResponseParam(response.getBody()).results.size()); //асерим не пустой параметр results
    ...
    @Test
    @UseDataProvider("dataForNegativeTest") //указываем тестовый метод и имя используемого дата провайдера
    public void negativeTest( String location, String radius, String keyword,
        ...
        assertEquals(200, response.getStatus()); //асертим статус код сервера
        assertEquals(statusMessage, JsonHelper.getResponseParam(response.getBody()).status); //асертим параметр status
        assertEquals(0, JsonHelper.getResponseParam(response.getBody()).results.size()); //асерим пустой параметр results
